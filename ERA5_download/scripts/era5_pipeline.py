...
This script orchestrates the complete ERA5 data processing workflow:
1. Download data from CDS
2. Preprocess and extract meteorological variables

Usage:
    python era5_pipeline.py --region central_europe --years 2020 2021 --variables base_variables
    python era5_pipeline.py --region iberia --years 2014-2021 --skip-download
    python era5_pipeline.py --region scandinavia --years 2020 --skip-preprocessing
"""
...
class ERA5Pipeline:
    """Main pipeline class for ERA5 data processing."""
    ...
    def run_download(self, years: List[str], variables: List[str]) -> None:
        ...
        cmd = [
            sys.executable,
            str(self.script_dir / "download_era5.py"),
            "--region", self.region,
            "--years"] + years + ["--variables", "all_variables"]
    ...
    def run_preprocessing(self, years: List[str] = None, variables: List[str] = None, 
        ...
        Args:
            ...
            input_dir: Input directory containing NetCDF files (optional, defaults to raw_data)
        """
        print("=" * 60)
        ...
        if years:
            for year in years:
                ...
                cmd = [
                    sys.executable,
                    str(self.script_dir / "preprocess_era5.py"),
                    "--region", self.region,
                    "--year", year,
    ...
    def run_full_pipeline(self, years: List[str], variables: List[str], 
                         skip_download: bool = False, skip_preprocessing: bool = False, 
                         input_dir: Path = None) -> None:
        ...
        Args:
            ...
            input_dir: Input directory for preprocessing (optional)
        """
        print("=" * 60)
        print(f"ERA5 DATA PROCESSING PIPELINE - {self.region.upper()}")
        print("=" * 60)
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            ...
            print(f"Warning: CDS API configuration issue: {e}")
            print("Please ensure your CDS API key is properly configured")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Complete ERA5 data processing pipeline",
...
Examples:
  ...
  # Skip download, only preprocess
  python era5_pipeline.py --region iberia --years 2014,2015 --skip-download
  
  # Skip preprocessing, only download
  python era5_pipeline.py --region scandinavia --years 2020 --skip-preprocessing
  
  # Process downloaded files with specific input directory
  python era5_pipeline.py --region central_europe --years 2021 --skip-download --input-dir /path/to/raw_data
  
  # Process different variable subsets from same downloaded data
  python era5_pipeline.py --region central_europe --years 2021 --skip-download --variables base_variables
  python era5_pipeline.py --region central_europe --years 2021 --skip-download --variables all_variables
        """
    )
    ...
    parser.add_argument(
        ...
        help="Variable group or comma-separated list of variables. "
             "Available groups: base_variables, all_variables"
    )
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        help="Input directory for preprocessing step (default: {region}/single_levels)"
    )
    
    args = parser.parse_args()
    
    # Check if this is a download-only operation
    download_only = args.skip_preprocessing
    ...
    if args.check_prerequisites:
        if not args.region:
            ...
            sys.exit(1)
        pipeline = ERA5Pipeline(args.region)
    ...
    # Initialize pipeline
    if not args.region:
        print("Error: --region is required for preprocessing steps")
        sys.exit(1)
    ...
    # Parse variables
    if args.variables in ["base_variables", "all_variables"]:
