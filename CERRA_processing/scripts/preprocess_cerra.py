...
class CERRAProcessorHDF5:
    ...
    def __init__(self, region: str, variables: str = "base_variables"):
        ...
        if variables == "base_variables":
            ...
            # Process all variables: u10, v10, t2m, sshf, zust
            self.variable_indices = [0, 1, 2, 3, 4]  # u10, v10, t2m, sshf, zust
            self.variable_names = ['u10', 'v10', 't2m', 'sshf', 'zust']
        else:
    ...
    def process_file_to_hdf5(self, input_file: Path, output_dir: Path, year: int) -> None:
        ...
        all_data_list = [u10, v10, t2m, sshf, zust]
        all_var_names = ['u10', 'v10', 't2m', 'sshf', 'zust']
        all_stacked_data = np.stack(all_data_list, axis=-1)  # (time, height, width, 5)
        
        # Extract only u10, v10, t2m (indices 0, 1, 2)
        selected_data = all_stacked_data[:, :, :, self.variable_indices]  # (time, height, width, 3)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process CERRA data to HDF5 format (u10, v10, t2m only)",
