...
This script creates train/validation/test splits from processed CERRA data across
...
Usage:
    ...
    python split_cerra_data.py --input-dir /path/to/CERRA_processing --output-dir /path/to/output
    
    # Use custom configuration file
    python split_cerra_data.py --config split_config.json --input-dir /path/to/CERRA_processing
"""
...
class CERRADataSplitter:
    """Class to handle splitting CERRA data across regions into train/val/test sets."""
    ...
    def __init__(self, input_dir: Path, output_dir: Path, split_config: Dict, 
        ...
        print(f"Initialized CERRA Data Splitter")
    ...
    def _get_available_years(self, region: str) -> List[int]:
        ...
        # Look for files matching pattern {region}_{year}_cerra.nc
        for file in region_dir.glob(f"{region}_*_cerra.nc"):
            # Extract year from filename
            try:
                parts = file.stem.split('_')
                if len(parts) >= 2 and parts[-1] == 'cerra':
                    year = int(parts[-2])
                    years.append(year)
    ...
    def _get_file_path(self, region: str, year: int) -> Path:
        ...
        """
        region_dir = self.regions[region]
        filename = f"{region}_{year}_cerra.nc"
    ...
    def _get_static_file_path(self, region: str) -> Path:
        ...
        """
        region_dir = self.regions[region]
        filename = f"{region}_static_cerra.nc"
        filepath = region_dir / filename
    ...
    def create_training_data(self) -> List[Tuple[str, xr.Dataset]]:
        ...
        for region, region_config in train_config.items():
            ...
            ds_region.attrs.update({
                'title': f'CERRA Training Dataset - {region}',
                'description': f'Training data for {region}',
                'region': region,
                'split': 'train',
                'creation_date': datetime.now().isoformat(),
                'creator': 'split_cerra_data.py',
    ...
    def create_validation_data(self) -> List[Tuple[str, xr.Dataset]]:
        ...
        for region, region_config in val_config.items():
            ...
            # Add metadata
            ds_region.attrs.update({
                'title': f'CERRA Validation Dataset - {region}',
    ...
    def create_test_data(self) -> List[Tuple[str, xr.Dataset]]:
        ...
        for region, region_config in test_config.items():
            ...
            # Add metadata
            ds_region.attrs.update({
                'title': f'CERRA Test Dataset - {region}',
    ...
    def create_static_data(self) -> List[Tuple[str, xr.Dataset]]:
        ...
        for region in region_names:
            ...
            # Add metadata
            ds.attrs.update({
                'title': f'CERRA Static Data - {region}',
                'description': f'Static variables (orography) for {region}',
                'region': region,
                'creation_date': datetime.now().isoformat(),
                'creator': 'split_cerra_data.py'
            })
    ...
    def run(self, skip_train: bool = False, skip_val: bool = False, 
        ...
        """
        print("\n" + "="*60)
        print("CERRA Data Splitting Pipeline")
        print("="*60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Split CERRA data into train/validation/test sets across multiple regions",
...
Configuration Format (JSON):
{
  "train": {
    "RegionName": {
      "directory": "latlon_proj_RegionName/single_levels_processed",
...
Output Structure (default: zz_processed_data/CERRA/):
  train/
    CentralEurope.nc
    Iberia.nc
    Scandinavia.nc
...
Examples:
  ...
  # Use custom configuration file
  python split_cerra_data.py --config my_split_config.json
  
  # For forecasting task (will show error with chunking guidance)
  python split_cerra_data.py --config my_config.json --task forecasting
  
  # Specify custom paths
  python split_cerra_data.py --config my_config.json --input-dir /path/to/CERRA_processing --output-dir /path/to/output
  
  # Create only training data
  python split_cerra_data.py --config my_config.json --skip-val --skip-test --skip-static
  
  # Use higher compression
  python split_cerra_data.py --config my_config.json --compression-level 9
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Root directory containing regional CERRA data subdirectories (default: ../CERRA_processing relative to script)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to save split datasets (default: ../zz_processed_data/CERRA)"
    )
    ...
    if args.input_dir is None:
        ...
        input_dir = script_dir.parent  # CERRA_processing directory
    else:
        input_dir = Path(args.input_dir)
    
    # Set default output directory
    if args.output_dir is None:
        output_dir = Path("../zz_processed_data/CERRA")
    else:
